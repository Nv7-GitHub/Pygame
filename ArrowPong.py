# Pong - Press Space to start. Up arrow key to move Right character up, Down arrow key to move left character down.
...
def render():
    ...
    # Pads
    pygame.draw.rect(win, (255, 255, 255), (gap, youy - height / 2, width, height))  # Your rectangle
...
while run:
    ...
    if keys[pygame.K_UP] and youy - abs(yvel) * 2 > 0:
        youy -= abs(yvel) * 2

    elif keys[pygame.K_DOWN] and youy + abs(yvel) * 2 < HEIGHT:
        youy += abs(yvel) * 2
    ...
    if x - xvel - radius < gap + width and y + yvel in range(int(youy - height / 2 - radius), int(youy + height / 2 + radius)):
        xvel *= -1
        x += radius
        score += 1
